...
void split_words(char *content, char ***words, int *word_count, int *words_capacity)
{
    ...
    *word_count = 0;
    *words_capacity = INITIAL_CAPACITY;
 ...
 * @param is_last_line A flag indicating whether this is the last line.
 * */
void printJustifiedLine(char **words, int word_count, int line_length, int is_last_line)
{
    if(is_last_line)
    {
        if(word_count == 1)
                          ...
                          int line_length)
{
    ...
    {
        printJustifiedLine(current_line_words, current_word_count, line_length, 1);
    }
}

/**
 * Handles words that are longer than the line length by splitting at hyphens.
 *
 * @param current_line_words Pointer to the array of words in the current line.
 * @param current_word_count Pointer to the number of words in the current line.
 * @param current_line_length Pointer to the current line length.
 * @param line_length The maximum number of characters per line.
 * @param word Pointer to the word to be processed (may be modified if split).
 * @return 0 if successful, 1 if an error occurs.
 * */
int processHyphenatedWord(char ***current_line_words, int *current_word_count,
                          size_t *current_line_length, int line_length, char **word)
{
    ...
    {
        // Find hyphen
        int hyphen_index = find_hyphen(*word, line_length - 1);
        if(hyphen_index > 0)
        {
            // Split the word
            ...
            strncpy(first_part, *word, hyphen_index + 1);
            first_part[hyphen_index + 1] = '\0';

            // Second part
            char *second_part = strdup(&(*word)[hyphen_index + 1]);
            if(second_part == NULL)
            {
                fprintf(stderr, ERROR_MEMORY_ALLOCATION_FAILED);
                free(first_part);
...
/**
 * Adds a word to the current line, justifying and resetting the line if necessary.
 *
 * @param current_line_words Pointer to the array of words in the current line.
 * @param current_word_count Pointer to the number of words in the current line.
 * @param current_line_length Pointer to the current line length.
 * @param current_line_capacity Pointer to the current capacity of the line words array.
 * @param line_length The maximum number of characters per line.
 * @param word The word to add to the line.
 * @return 0 if successful
 *         1 if an error occurs
 * */
int add_word_to_line(char ***current_line_words, int *current_word_count,
                     size_t *current_line_length, int *current_line_capacity,
                     int line_length, char *word)
{
    size_t word_len = strlen(word);
    ...
    if(*current_line_length + extra_space + word_len <= line_length)
    {
        ...
        (*current_line_words)[(*current_word_count) ++] = word;
        *current_line_length += word_len;
    } else
    {
        printJustifiedLine(*current_line_words, *current_word_count, line_length, 0);
...
int format_text(char *content, int line_length)
{
    ...
    // Init line
    char **current_line_words = malloc(INITIAL_CAPACITY * sizeof(char *));
    ...
    int current_line_capacity = INITIAL_CAPACITY;
    ...
    for(int word_index = 0; word_index < word_count; word_index ++)
    {
        ...
        // Check if word can be added to the line
        size_t word_len = strlen(words[word_index]);

        // Handle words longer than the line length
        while(word_len > line_length)
        {
            // Handle long words by splitting at hyphens
            if(processHyphenatedWord(&current_line_words, &current_word_count,
        ...
        if(add_word_to_line(&current_line_words, &current_word_count,
                            &current_line_length, &current_line_capacity,
                            line_length, words[word_index]) != 0)
        {
            error_flag = 1;
            break;
        }
