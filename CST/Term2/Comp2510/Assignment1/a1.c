...
void split_words(char *content, char ***words, int *word_count, int *words_capacity)
{
    ...
    *words = malloc(*words_capacity * sizeof(char *));
    ...
    while(content_char_index < content_len)
    {
        // Skip leading whitespace
        /* Function doc:
         * int isspace(int ch);
         * ch: char character, but implicit conversion to int. Better to use unsigned char
         *     to avoid undefined behavior when ch is negative
         * return 1 if ch is a whitespace character
         *        0 otherwise
         */
        while(content_char_index < content_len
              && isspace((unsigned char) content[content_char_index]))
        {
            content_char_index ++;
        }

        int word_start = content_char_index;
        while(content_char_index < content_len &&
              ! isspace((unsigned char) content[content_char_index]))
        {
            content_char_index ++;
        }
        int word_end = content_char_index;
...
/**
 * Searches for a hyphen character within a word.
 *
 * This function scans the input word and returns the index of the first hyphen found.
 *
 * @param word The word in which to search for a hyphen.
 *
 * @return The index of the hyphen if found; otherwise, returns -1.
 */
int find_hyphen(const char *word)
{
...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, "Error: Memory allocation failed for current_line_words.\n");
        return 1;
    }
    ...
    for(int i = 0; i < word_count; i ++)
    {
        if(words[i] == NULL)
        {
            fprintf(stderr, "Error: Word at index %d is NULL.\n", i);
            continue;
        }
        ...
        if(current_line_length + extra_space + word_len <= line_length)
        {
            ...
            if(current_word_count >= current_line_capacity)
            {
                ...
                {
                    fprintf(stderr,
                            "Error: Memory allocation failed during reallocation.\n");
                    return 1;
                }
                current_line_words = temp;
            }

            current_line_words[current_word_count ++] = words[i];
